...
public class MethodServiceTest
{
    ...
    public void AddParameter_WhenValid_ShouldAddToMethod()
    {
        ...
        var result = service.AddParameter(methodId, newParam);

        result.Should().NotBeNull();
        result.Name.Should().Be("testParam1");
        method.Parameters.Should().ContainSingle(p => p.Name == "testParam1");
        methodRepoMock.Verify(r => r.Update(method), Times.Once);
    }
